<template>
  <div class="app-container">
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <el-button size="mini" class="btn-add" @click="handleShowCategory()">资源分类
      </el-button>
    </el-card>
...
<script>
// import { fetchList, saveOrUpdate, deleteResource } from '@/services/resource'
  ...
  methods: {
    ...
    handleDelete (index, row) {
      this.$confirm('是否要删除该资源?', '提示', {
        ...
        bossApi.deleteResource(row.id).then((response) => {
          this.$message({
            type: 'success',
            message: '删除成功!',
          })
    ...
    handleDialogConfirm () {
      if (this.isEdit) {
        bossApi.saveOrUpdateResource(this.resource).then((res) => {
          this.$message({
            message: '修改成功！',
            type: 'success',
          })
    ...
    handleShowCategory () {
      this.$router.push({ path: '/permission/resourceCategory' })
    },
    getList () {
      this.listLoading = true
      bossApi.fetchResourceList(this.listQuery).then((res) => {
    ...
    getCateList () {
      bossApi.listAllCate().then((res) => {
        const cateList = res.response
