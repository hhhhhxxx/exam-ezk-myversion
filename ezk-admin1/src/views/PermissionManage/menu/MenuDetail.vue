...
export default {
  ...
  created () {
    bossApi.getMenu(this.$route.query.id || -1).then(res => {
      if (this.isEdit) {
        this.menu = res.response.menuInfo
      } else {
        this.menu = Object.assign({}, defaultMenu)
      }
      this.selectMenuList = res.response.parentMenuList.map(item => {
  ...
  methods: {
    getSelectMenuList () {
      bossApi.fetchAllMenuList(0, { pageSize: 100, pageNum: 1 }).then(res => {
        this.selectMenuList = res.response
        // console.log(this.selectMenuList.length)
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$confirm('是否提交数据', '提示', {
            ...
            if (this.isEdit) {
              bossApi.saveOrUpdateMenu(this.menu).then(res => {
                this.$message({
                  message: '修改成功',
                  type: 'success',
                  duration: 1000
