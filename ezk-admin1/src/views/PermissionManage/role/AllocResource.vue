...
export default {
  ...
  methods: {
    getAllResourceList () {
      bossApi.fetchAllResourceList().then(res => {
        this.allResource = res.response
    ...
    getAllResourceCateList () {
      bossApi.listAllCate(this.$route.query.roleId).then(res => {
        this.allResourceCate = res.response
    ...
    getResourceByRole (roleId) {
      bossApi.listResourceByRole(roleId).then(res => {
        const allocResource = res.response
    ...
    handleSave () {
      this.$confirm('是否分配资源？', '提示', {
        ...
        bossApi.allocResource(params).then(res => {
          this.$message({
            message: '分配成功',
            type: 'success',
            duration: 1000
