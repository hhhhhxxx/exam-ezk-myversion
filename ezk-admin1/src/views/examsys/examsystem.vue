            ...
            <div class="jinggao-info-class">
              <div class="tongji-class">
                <el-descriptions class="margin-top" title="统计信息：" :column="1" size="small" border>
                  ...
                  <el-descriptions-item>
                    <template slot="label">活体：</template>
                    <p v-if="isLive !== true" style="color: red">{{ isLive === true ? '是' : '否' }}</p>
                    <p v-if="isLive === true">{{ isLive === true ? '是' : '否' }}</p>
                  </el-descriptions-item>
                  <el-descriptions-item>
                    <template slot="label">人数变化次数:</template>
                    <p>{{ peopleNumChangeCount }}</p>
                  </el-descriptions-item>
                  <el-descriptions-item>
                    <template slot="label">活体变化次数:</template>
                    <p>{{ nonLiveChangeCount }}</p>
                  </el-descriptions-item>
                  <el-descriptions-item>
                    <template slot="label">屏幕异常次数:</template>
                    <p>{{ exitFullCount }}</p>
                  </el-descriptions-item>
                </el-descriptions>
      ...
      <el-col :span="8">
        <div class="operate-class">
          ...
          <div class="send-msg-class">
            ...
            <el-input type="textarea" :rows="1" placeholder="请输入内容" v-model="warnMsg"></el-input>
            <div>
...
export default {
  ...
  created () {
    ...
    // })
    this.examInfoTimer = setInterval(this.getExamInfoData, 5000)
  },

  beforeDestroy () {
    // 关闭定时器
    clearInterval(this.examInfoTimer)
  },
  methods: {
    // 定时获取考生监控数据
    getExamInfoData () {
    ...
    async detectFace (image) {
      ...
      // 人脸识别数据
      let data = {
        // canvas.toDataURL：返回值是一个数据url，是base64组成的图片的源数据、可以直接赋值给图片的src属性。
      ...
      this.$refs.img.src = canvas.toDataURL('image/jpg')
    ...
    initWebSocket () {
      ...
      this.ws.onmessage = (ev) => {
        ...
        if (json.code === '1') {
          ...
          console.log('选项框里选中的考生，才展示监控画面', this.choiceUser, json.fromName)
