...
export default {
  ...
  methods: {
    getAllResourceList () {
      bossApi.fetchAllResourceList().then(response => {
    ...
    getAllResourceCateList () {
      bossApi.listAllCate(this.$route.query.roleId).then(response => {
    ...
    getResourceByRole (roleId) {
      bossApi.listResourceByRole(roleId).then(response => {
    ...
    handleSave () {
      this.$confirm('是否分配资源？', '提示', {
        ...
        bossApi.allocResource(params).then(response => {
          this.$message({
            message: '分配成功',
            type: 'success',
            duration: 1000
