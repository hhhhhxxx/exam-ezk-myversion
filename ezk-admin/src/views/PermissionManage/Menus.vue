...
export default {
  ...
  methods: {
    ...
    handleAddMenu() {
      this.$router.push("/permission/addMenu");
    },
    getList() {
      this.listLoading = true;
      bossApi.fetchAllMenuList(this.parentId, this.listQuery).then((response) => {
    ...
    handleDelete(index, row) {
      this.$confirm("是否要删除该菜单", "提示", {
        ...
        bossApi.deleteMenu(row.id).then((response) => {
          this.$message({
            message: "删除成功",
            type: "success",
            duration: 1000,
