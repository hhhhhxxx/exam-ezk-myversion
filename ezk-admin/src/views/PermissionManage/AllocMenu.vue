...
export default {
  ...
  methods: {
    treeList () {
      bossApi.getMenuNodeList().then(response => {
        this.menuTreeList = JSON.parse(JSON.stringify(response.data))
      })
    },
    getRoleMenu (roleId) {
      bossApi.listMenuByRole(roleId).then(response => {
    ...
    handleSave () {
      ...
      this.$confirm('是否分配菜单？', '提示', {
        ...
        bossApi.allocMenu(params).then(response => {
          this.$message({
            message: '分配成功',
            type: 'success',
            duration: 1000
